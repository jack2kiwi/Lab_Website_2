...
function loadStatsPage(){
	...
	for(row_counter = 2; row_counter < table.rows.length; row_counter++){
		...
		if(parseInt(table.rows[row_counter].cells[3].innerHTML) > parseInt(table.rows[row_counter].cells[2].innerHTML)){
	...
	document.getElementById("losses").innerHTML = lose_count;
}
...
function loadPlayersPage(){
	...
	for(var i = 0; i < players.length; i++){
		...
		anchor.index = i;
		anchor.onclick = function() {switchPlayers(this.index)};
